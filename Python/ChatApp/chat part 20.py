...
class App(ctk.CTk):
    ...
    def bt_send_com(self,e=None):
        ...
        if self.index_mode==0:
            ...
            #print(self.recording1)
            wv.write(f"ChatFolder\\{self.my_phone}\\{self.phones[self.index_chat]}\\records\\{self.timeNow.year}-{self.timeNow.month}-{self.timeNow.day}-{self.timeNow.hour}-{self.timeNow.minute}-{self.timeNow.second}.wav", self.recording1[0:int(self.Exactduration*freq)], freq, sampwidth=2)
    ...
    def bt_send_rec_com(self):
        ...
        self.list_chat[self.index_chat].append({'M':f"ChatFolder\\{self.my_phone}\\{self.phones[self.index_chat]}\\records\\{self.timeNow.year}-{self.timeNow.month}-{self.timeNow.day}-{self.timeNow.hour}-{self.timeNow.minute}-{self.timeNow.second}.wav",'R':True,'seen':False,'time':timeNowStr,'I':False})
    ...
    def get_chats_Net(self):
        direct = f'ChatFolder\\{self.my_phone}'
        ...
        self.phones = list(data.keys())
        
        self.list_chat=[]
        
        for phone in self.phones:
            direct_chat = direct+f'\\{phone}\\chat.json'
            f = open(direct_chat)
            data = json.load(f)
            f.close()
    ...
    def bt_send_image_com(self):
        ...
        for i in range(0,loop):
            ...
            for image in self.images:
                ...
                shutil.copy(image, f"ChatFolder\\{self.my_phone}\\{self.phones[self.index_chat]}\\images\\{timeNow.year}-{timeNow.month}-{timeNow.day}-{timeNow.hour}-{timeNow.minute}-{timeNow.second}-{count}.{extension}")
                count+=1
            self.list_frames.append(self.bsk_frame)
